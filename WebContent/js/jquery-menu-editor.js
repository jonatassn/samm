 ...
 * 
 * Autor original
 * David Ticona Saravia https://github.com/davicotico
 * 
 * Adaptado por 
 * Alison R. P. Freitas (21/03/2021)
 * 
 * As adaptações aqui consiste em:
 * - Guardar em um array os itens excluídos, para poder efetuar a exclusão no backend. Verificar onde tem "arrExcluidos" para editar. (funcionalidade para manipulação backend)
 * - Adicionado um espaço para subsituir o espaço ocupado pelo botão +- de expasão da árvore, caso o item não tenha filhos. Verificar onde tem "openerspace" para editar. (melhoria visual)
 * - Removido a funcionalidade iconPicker. Verificar onde tem "iconPicker" para editar. (funcionalidade não utilizada)
 * 
 * */
( function( $ )
{    
    ...
    $.fn.sortableLists = function( options )
    {
        ...
        {
            console.error('Parâmetros inválidos para opener.as');
        }
        ...
        {
            if ( ! setting.opener.open ) throw 'Opener.open value não definido. Isso deve ser uma url, html or css class válida.';
            if ( ! setting.opener.close ) throw 'Opener.open value não definido. Isso deve ser uma url, html or css class válida.';
        ...
        function endDrag( e )
        {
            ...
            cEl.el.animate( { left: offset.left - state.cEl.mL, top: offset.top - state.cEl.mT }, 250,
                function()  // complete callback
                {
                    ...
                    {
                        // remove o espaco (.openerspace) antes de por o opener
                        hintWrapperNode.prev( 'div' ).first().find('.openerspace').first().remove();
            ...
            $( setting.listSelector, state.rootEl.el ).each( function( i )
                {
                    ...
                    {
                        // adiciona espaco antes de remover o toOpener
                        var openerSpace = $('<span>').addClass('openerspace').css({'padding-left': '3em','line-height': '2em'});
                        openerSpace.prependTo($( this ).prev( 'div' ).first());
...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        labelEdit: '<i class="z-icon-pencil clickable"></i>',
        labelRemove: '<i class="z-icon-trash clickable"></i>',
        textConfirmDelete: 'Tem certeza que deseja apagar o item e todos os seus filhos?',
        ...
        listOptions: { 
            ...
            opener: {
                as: 'html',
                close: '<i class="z-icon-minus"></i>',
                open: '<i class="z-icon-plus"></i>',
                openerCss: {'margin-right': '10px', 'float': 'none'},
                openerClass: 'btn btn-light btn-sm',
            },
    ...
    $(document).on('click', '.btnRemove', function (e) {
        ...
        if (confirm(settings.textConfirmDelete)) {
            ...
            guardaExcluido($(this).closest('li'));
            ...
            if ((!list.children().length) && (!isMainContainer)) {
                ...
                // adiciona espaco se remover o toOpener
                var openerSpace = $('<span>').addClass('openerspace').css({'padding-left': '3em','line-height': '2em'});
                openerSpace.prependTo($liParent.children('div').first());
            }
            MenuEditor.updateButtons($main);
        }
    });
    ...
    $main.on('click', '.btnIn', function (e) {
        ...
        if ($prev.length > 0) {
            ...
            if ($ul.length > 0) {
                ...
                // remove o espaco (.openerspace) se adicioanr o tOpener
                $prev.children('div').first().find('.openerspace').first().remove();
     ...
     * @return {array} Array
     */
    function guardaExcluido(liExcluido) {
        var arr = [];
        liExcluido.each(function () {
            var li = $(this);
            var object = li.data();
            arr.push(object);
            var ch = li.children('ul,ol').sortableListsToJson();
    ...
    function stringToArray(str) {
        try {
            ...
            console.log('A string não é um formato Json válido');
            return null;
        }
        return obj;
    }
    ...
    function TButtonGroup() {
        ...
        var $btnUp = TButton({classCss: 'btn btn-light btn-sm btnUp btnMove', text: '<i class="z-icon-angle-up clickable"></i>'});
        var $btnDown = TButton({classCss: 'btn btn-light btn-sm btnDown btnMove', text: '<i class="z-icon-angle-down clickable"></i>'});
        var $btnOut = TButton({classCss: 'btn btn-light btn-sm btnOut btnMove', text: '<i class="z-icon-level-down clickable"></i>'});
        var $btnIn = TButton({classCss: 'btn btn-light btn-sm btnIn btnMove', text: '<i class="z-icon-level-up clickable"></i>'});
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $span = $('<span>').addClass('txt').append(v.nome).css('margin-right', '5px');
    ...
    function setOpeners() {
        $main.find('li').each(function () {
            ...
            if ($li.children('ul').length) {
                ...
                //Espaçamento para compensar a falta de botão plus
                var openerSpace = $('<span>').addClass('openerspace').css({'padding-left': '3em','line-height': '2em'});
                openerSpace.prependTo($li.children('div').first());
            }
        });
    }
    ...
    this.update = function(){
        ...
        $form.find('.item-menu').each(function() {
            if($(this).val()) {
                ...
                $cEl.data($(this).attr('name'), null);
            }
        });
        $cEl.children().children('i').removeClass(oldIcon).addClass($cEl.data('icon'));
        $cEl.find('span.txt').first().text($cEl.data('nome'));
        resetForm();
    };
    ...
    this.add = function(){
        ...
        var textItem = $('<span>').addClass('txt').text(data.nome);
        var iconItem = $('<i>').addClass("");
    ...
    this.setExcluidos = function (strJson) {
        var arrayItem = (Array.isArray(strJson)) ? strJson : stringToArray(strJson);
        if (arrayItem !== null) {
            arrExcluidos = arrayItem;
        }
    };
    
    this.getExcluidos = function () {
        return arrExcluidos;
    };
    
    this.getExcluidosString = function () {
        return JSON.stringify(arrExcluidos);
    };
};
/* STATIC METHOD */
